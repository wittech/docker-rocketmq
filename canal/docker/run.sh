...
function usage() {
    ...
    echo "  run.sh [CONFIG]"
    echo "example 1 :"
    echo "  run.sh -e canal.instance.master.address=192.168.253.37:3306 \\"
    ...
    echo "         -e canal.mq.topic=all_wit \\"
    echo "         -e canal.instance.filter.regex=wit_ccp\\\\.ccp.* "
    echo "example 2 :"
...
CONFIG=${@:1}
VOLUMNS="-v /home/canal/logs:/home/admin/canal-server/logs"
...
MEMORY="-m 4096m"
LOCALHOST=`getMyIp`
cmd="docker run -d -it -h $LOCALHOST --restart=always $CONFIG --name=canal-server $VOLUMNS $NET_MODE $PORTS $MEMORY canal/canal-server:latest"
echo $cmd
eval $cmd
