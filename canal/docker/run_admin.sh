...
function usage() {
    ...
    echo "         -e canal.adminUser=withub \\"
    echo "         -e canal.adminPasswd=4ACFE3202A5FF5CF467898FC58AAB1D615029"
    exit
}
...
MEMORY="-m 1024m"
LOCALHOST=`getMyIp`
cmd="docker run -d -it -h $LOCALHOST --restart=always $CONFIG --name=canal-admin $VOLUMNS $NET_MODE $PORTS $MEMORY canal/canal-admin:latest"
echo $cmd
eval $cmd